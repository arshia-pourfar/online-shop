...
export default function CustomersPage() {
    ...
    useEffect(() => {
        ...
        getCustomerStatuses().then(setCustomerStatuses); // Re-added
    }, []);
                    ...
                    <table className="min-w-full text-sm text-left table-fixed">
                        <thead className="bg-primary-bg text-accent select-none">
                            <tr>
                                ...
                                <SortableTH
                                    title="role"
                                    onClick={() => onSortClick("role")} // Re-added Phone column
                                    sortable={true}
                                    sortAsc={sortAsc}
                                    isActive={sortField === "role"}
                                    width="w-36"
                                />
                                <SortableTH
                                    title="Phone"
                            ...
                            {paginatedCustomers.length === 0 ? (
                                ...
                                paginatedCustomers.map((user) => (
                                    <tr key={user.id} className="border-b last:border-none hover:bg-primary-bg/50 transition">
                                        ...
                                        {/* <td className={`px-2 py-2 ${user.role === "ADMIN" ? "bg-status-positive" : "bg-status-hidden"}`}> */}
                                        <td className="px-2 py-2">
                                            <CustomerStatusBadge status={user.role} />
                                        </td>
                                        <td className="px-2 py-2">{user.phone || "N/A"}</td>
                                        <td className="px-2 py-2">
                                            {/* Safely pass status to StatusBadge */}
                                            <CustomerStatusBadge status={user.status || "UNKNOWN"} />
                                        </td>
